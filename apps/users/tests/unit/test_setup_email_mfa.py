"""Unit tests for SetupEmailMFAUseCase."""

from __future__ import annotations

from unittest.mock import MagicMock

import pytest

from apps.users.application.use_cases.setup_email_mfa import SetupEmailMFAUseCase
from apps.users.domain.exceptions import MFAAlreadyEnabledError
from apps.users.tests.unit.fakes import FakeUserRepository, make_user


def _repo(user):
    return FakeUserRepository([user])


def test_setup_email_generates_otp_and_publishes_event():
    user = make_user(email="u@example.com", first_name="Alice")
    otp = MagicMock()
    otp.generate_and_store.return_value = "OTP99999"
    events = MagicMock()

    SetupEmailMFAUseCase(_repo(user), otp, events).execute(user.id)

    otp.generate_and_store.assert_called_once_with(user.id)
    events.publish.assert_called_once()
    call_args = events.publish.call_args[0]
    assert call_args[0] == "iam.mfa_email_otp_requested"
    assert call_args[1]["email"] == "u@example.com"
    assert call_args[1]["first_name"] == "Alice"
    assert call_args[1]["otp"] == "OTP99999"


def test_setup_email_mfa_already_enabled_raises():
    user = make_user(mfa_enabled=True)
    with pytest.raises(MFAAlreadyEnabledError):
        SetupEmailMFAUseCase(_repo(user), MagicMock(), MagicMock()).execute(user.id)
